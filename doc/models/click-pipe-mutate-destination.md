...
| `roles` | `List[str]` | Optional | Optional. Roles to grant to the ClickHouse user that ClickPipe creates. If omitted, the user is granted the default role (`default_role`). Add your custom roles here if required. The role names `clickpipes` and `clickpipes_system` are reserved for internal use and cannot be assigned. |
...
click_pipe_mutate_destination = ClickPipeMutateDestination(
    database='database8',
    table='table6',
    managed_table=False,
