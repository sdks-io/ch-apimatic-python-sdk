
# Udf Version Create Request 2

## Data Type

`UdfVersionCreateRequest | UdfVersionCreateRequest1`

## Cases

| Type |
|  --- |
| [`UdfVersionCreateRequest`](../../../doc/models/udf-version-create-request.md) |
| [`UdfVersionCreateRequest1`](../../../doc/models/udf-version-create-request-1.md) |

## UdfVersionCreateRequest

### Initialization Code

#### Example

```python
value = UdfVersionCreateRequest(
    upload_id='0000139a-0000-0000-0000-000000000000',
    runtime=Runtime.ENUM_PYTHON311,
    arguments=[
        UdfArgument(
            name='name8',
            mtype='type2'
        )
    ],
    return_type='returnType6',
    command_read_timeout=10000,
    command_write_timeout=10000,
    send_chunk_header=False,
    deterministic=False,
    format='TabSeparated',
    sandbox_type=SandboxType.BASIC,
    sandbox_version=SandboxVersion.V2
)
```

## UdfVersionCreateRequest1

### Initialization Code

#### Example

```python
value = UdfVersionCreateRequest1(
    upload_id='000001be-0000-0000-0000-000000000000',
    runtime=Runtime.ENUM_PYTHON311,
    arguments=[
        UdfArgument(
            name='name8',
            mtype='type2'
        )
    ],
    return_type='returnType8',
    command_read_timeout=10000,
    command_write_timeout=10000,
    send_chunk_header=False,
    deterministic=False,
    format='TabSeparated',
    sandbox_type=SandboxType.BASIC,
    sandbox_version=SandboxVersion.V2,
    pool_size=3,
    max_command_execution_time=10
)
```

