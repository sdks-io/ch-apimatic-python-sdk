...
class PostgresService(SdkBaseModel):
    ...
    storage_size: Optional[int] = Field(default=UNSET, alias="storageSize")
    ...
    """Connection string to the Postgres service. Embeds the service password, so it is only returned when the service
    is created or its password is reset. Omitted from every other response when Postgres credential redaction is enabled
    for the organization. Not guaranteed to be present — treat as optional."""

    username: Optional[str] = UNSET
    """Username for the Postgres service"""

    password: Optional[str] = UNSET
    """Password for the Postgres service. Only returned when the service is created or its password is reset. Omitted
    from every other response when Postgres credential redaction is enabled for the organization. Not guaranteed to be
    present — treat as optional."""

    hostname: Optional[str] = UNSET
    """Hostname for the Postgres service"""


class PostgresServiceDict(TypedDict):
